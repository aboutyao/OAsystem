...
每次写代码/配置/部署时，必须问自己：
1. **这个改动会触发什么连锁反应？** （Docker 缓存、数据库迁移、CORS、端口冲突）
2. **什么条件下会出错？** （空值、并发、权限、网络断开）
3. **用户会怎么用？** （公网访问、移动端、不同角色）
4. **部署后怎么验证？** （编译、类型检查、API 测试、端到端验证）

**常见预判清单：**
- 改配置 → 谁依赖它？改了会不会断？
- 新增 API → 前端调用了吗？CORS 允许了吗？
- 新增迁移 → ID 冲突了吗？外键约束了吗？
- 新增组件 → import 完整吗？CSS 变量用了吗？
- Docker 构建 → 缓存会跳过吗？用 --no-cache 吗？
