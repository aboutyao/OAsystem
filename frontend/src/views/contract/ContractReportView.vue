...
<template>
  <div class="oa-page" v-loading="loading">
    <div class="oa-page__head">
      <div>
        <h2 class="oa-page__title">合同报表</h2>
        <p class="muted">合同数据统计与分析。</p>
      </div>
    </div>
    ...
    <template v-if="data">
      <div class="report-cards">
        <el-card shadow="never">
          <div class="metric">
            <div class="metric__label">合同数</div>
            <div class="metric__value">{{ data.contractCount ?? 0 }}</div>
          </div>
        </el-card>
        <el-card shadow="never">
          <div class="metric">
            <div class="metric__label">合同总金额</div>
            <div class="metric__value">&yen;{{ data.totalAmount ?? 0 }}</div>
          </div>
        </el-card>
        <el-card shadow="never">
          <div class="metric">
            <div class="metric__label">30 天内到期</div>
            <div class="metric__value">{{ data.expiringSoon30Days ?? 0 }}</div>
          </div>
        </el-card>
      </div>
      ...
      <el-card shadow="never" style="margin-top: 16px">
        ...
        <el-table :data="(data.byType as JsonObject[]) ?? []" stripe>
          <el-table-column prop="contractType" label="类型" min-width="160" />
